...
public class Main_Gradle extends Activity {
     ...
     *
     * Android应用签名有三种版本：v1、v2和v3。这些签名版本用于验证应用的完整性和来源，并确保应用在发布和安装过程中的安全性。
     *
     * 1. v1签名：
     * - v1签名是最早引入的签名版本，使用JAR签名格式。
     * - 它包含应用的整个内容的数字签名，但不包括应用的资源文件。
     * - v1签名在Android 7.0及更早版本中是必需的。
     *
     * 2. v2签名：
     * - v2签名是Android 7.0引入的新签名格式，使用APK签名格式。
     * - 它包含应用的整个内容的数字签名，包括应用的资源文件。
     * - v2签名提供了更好的安全性和完整性验证，并减少了应用安装时的验证时间。
     * - 从Android 7.0开始，v2签名成为了默认的签名格式。
     *
     * 3. v3签名：
     * - v3签名是Android 9.0引入的新签名格式，也使用APK签名格式。
     * - 它在v2签名的基础上增加了对应用的额外信息的签名，以提供更强的安全性保障。
     * - v3签名进一步加强了应用的完整性验证和安全性。
     * v3签名中的额外信息指的是应用的附加元数据，这些元数据可能包括应用的版本号、发布渠道、签名版本、构建时间等信息。
     *
     *
     * 在发布应用时，建议使用最新的签名版本（v3签名），以提供最佳的安全性和完整性保障。Android Studio和Google Play Console提供了相应的工具和支持，可以方便地对应用进行v2和v3签名。
     *
     *资源文件
     * 1. 布局文件：XML格式的文件，用于定义应用界面的布局结构和组件的摆放位置。
     *
     * 2. 图像文件：包括应用中使用的各种图片资源，如PNG、JPEG等格式的图像文件。
     *
     * 3. 字符串资源：用于存储应用中使用的各种文本字符串，例如界面上的标签文本、按钮文本等。
     *
     * 4. 颜色和样式资源：用于定义应用中使用的颜色值和样式信息，以便在应用中进行统一的颜色和样式管理。
     *
     * 5. 动画和drawable资源：用于定义应用中使用的动画效果和可绘制对象，如矢量图形、形状等。
     *
     * ==============apksigner===========
