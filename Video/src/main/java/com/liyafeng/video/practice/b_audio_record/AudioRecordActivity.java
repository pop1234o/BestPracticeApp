...
public class AudioRecordActivity extends Activity {
     ...
     * <p>
     * <p>
     * 采样精度:一个采样数据可以是16位，也可以是24或者其他，这就是采样精度。
     * <p>
     ...
     *
     * ===============采样数=============
     * 1. 要使采集到的波形信号不失真，得满足香农定理。简单的说，就是你（仪器）采样频率必须大于被测信号
     * 频率的2陪。在实际中，我们把采样频率设置为信号频率的20~50倍，采集到的信号就很好了。
     * 2. 关于采样数，是指你的仪器采用有限采集模式时，你需要的采样点数。比如，当你把仪器的采样频率设为
     * 10KHz，你把采样点数设置为100K，那么你的仪器将采集10秒钟。当采用频率设置好后，采样点数可以用
     * 来控制你仪器的采样时间。同时，采样点数也决定了buff空间的大小，设置仪器的buff值（如果有的话）必须
     * 大于你采样点数。
     *
     * 采样率*时间（秒）=采样数
     *
